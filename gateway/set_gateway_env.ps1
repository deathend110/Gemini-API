...
Write-Host "  uv sync"
Write-Host "  如已有可用 cookies.json，可直接运行 uv run python -m gateway.main"
Write-Host "  如缺少可用 cookies.json 或需要主动刷新登录态，再运行 uv sync --extra browser"
Write-Host "  uv run --extra browser python -m gateway.refresh_cookies"
Write-Host "  如提示缺少调试会话，请先关闭当前已普通打开的同 profile Chrome 窗口，再用该命令重新启动"
